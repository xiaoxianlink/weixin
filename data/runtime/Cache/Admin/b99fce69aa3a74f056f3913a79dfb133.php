...
.nav-list>li .submenu>li>a {background: #37414a; color: #91a0ad}
...
function select_menu(i,j) {
	$(".img").attr('src',"/statics/simpleboot/themes/bluesky/ty0.png");
	$(".mts").css("color","#91a0ad");
	$("#img_"+i+"_"+j).attr('src',"/statics/simpleboot/themes/bluesky/ty"+i+".png");
	$("#span_"+i+"_"+j).css("color","#fff");
}
</script>
...
<?php $i ++; ?>
					<li>
						<?php if(empty($menu->items)){ ?>
							<a class="menu<?php echo ($i); ?>" href="javascript:openapp('<?php echo ($menu->url); ?>','<?php echo ($menu->id); ?>','<?php echo ($menu->name); ?>');">
								<i class="fa fa-<?php echo ((isset($menu->icon) && ($menu->icon !== ""))?($menu->icon):'desktop'); ?>"></i>
								<span class="menu-text"><?php echo ($menu->name); ?></span>
							</a>
						<?php }else{ ?>
							<a href="#" class="dropdown-toggle menu<?php echo ($i); ?>">
								...
								<b class="arrow fa fa-angle-right normal" style="color:#fff;"></b>
							...
							<ul  class="submenu">
									<?php getsubmenu1((array)$menu->items, $i) ?>
							</ul>	
						<?php } ?>
...
<?php function getsubmenu1($submenus, $i){ ?>
<?php $j = 0; ?>
<?php foreach($submenus as $menu){ ?>
<?php $j ++; ?>
					<li>
						<?php if(empty($menu->items)){ ?>
							<a href="javascript:openapp('<?php echo ($menu->url); ?>','<?php echo ($menu->id); ?>','<?php echo ($menu->name); ?>');" onclick="select_menu(<?php echo ($i); ?>,<?php echo ($j); ?>)">
								<i class="fa"><img id="img_<?php echo ($i); ?>_<?php echo ($j); ?>" class="img" src="/statics/simpleboot/themes/bluesky/ty0.png" style="margin-bottom: 4px;"></i>
								<span class="menu-text mts" id="span_<?php echo ($i); ?>_<?php echo ($j); ?>"><?php echo ($menu->name); ?></span>
							</a>
						<?php }else{ ?>
	...
	<div class="main-container container-fluid">
		...
		<div class="sidebar" id="sidebar">
			...
			<ul class="nav nav-list">
				<li style="height: 70px; line-height: 70px; text-align: center; color: #fff; font-size: 20px;">紫牛系统</li>
				<?php echo getsubmenu($submenus);?>
			</ul>
			</div>
			<div style="height: 50px; line-height: 50px; text-align: center; color: #fff; font-size: 18px;">已登录：<?php echo ((isset($admin["user_nicename"]) && ($admin["user_nicename"] !== ""))?($admin["user_nicename"]):$admin[user_login]); ?>&nbsp&nbsp&nbsp<a href="<?php echo U('Public/logout');?>" style="color: #fff;"></i> 退出</a></div>
		</div>
		...
		<div class="main-content">
			...
			<div class="page-content" id="content">
				<iframe src="<?php echo U('Main/index');?>" style="width:100%;height: 120%;" frameborder="0" id="appiframe-0" class="appiframe"></iframe>
			</div>
		</div>
	</div>
